...
export default function ExchangeRatesCard() {
  ...
  const { eurVes, isManual, lastFetch, lastError, autoEurVes, setRate } = useExchangeRate();
  ...
  const syncNow = async () => {
    ...
    try {
      ...
      if (res.data?.ok) toast.success('Tasa del BCV actualizada');
      else toast.error('El BCV no devolvió una tasa nueva. Se conserva la anterior.');
  ...
  const saveManual = async (enabled) => {
    setBusy(true);
    try {
      await setRate({ eurVes: parseFloat(manualEur) || autoEurVes, enabled });
      setManual(enabled);
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle className="flex items-center gap-2">
              <Coins className="h-5 w-5 text-primary" /> Tasa de cambio (BCV)
            </CardTitle>
            <CardDescription>
              Se sincroniza automáticamente dos veces al día y la comparten todas las cajas.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-5">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-3">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Paridad de cobro</p>
            <p className="text-xl font-bold font-mono text-primary">1 $ = 1 €</p>
          </div>
        </div>
